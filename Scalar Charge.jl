...
@progress (name="Saving data to file...")for i in range(0,48,step=8)
    Tindex=i
    ...
    for j in range(1,length(readdir()),step=1)
        ...
        @progress (name="T=$Tindex, μₐ=$(748+16*i), $rdir ") for i in range(0,42,step=1)
            findex = 748 + i * 16
            try
        ...
        global dataoutfile = open("ScalarChargeData.txt","a")
